...
router.get('/', (req, res, next) => {
    ...
    if (str.length == '')
        ...
        arrayPages = ['Google Scholar',
            'Redalyc', 'Microsoft Academic', 
            'Dialnet', 'Scholarpedia', 'Springlink', 
            'Redlatinos', 
            'Scielo', 
            'Reefseek', 
            'Cern', 
        ...
        const paginasPrincipal = {
            ...
            'UNI': 'http://cybertesis.uni.edu.pe/simple-search?query=',
            'UNMSM': 'http://cybertesis.unmsm.edu.pe/discover?scope=%2F&query=',
            'U. Pacífico': 'http://repositorio.up.edu.pe/discover?scope=%2F&query=',
            'U. Cayetano Heredia': 'http://repositorio.upch.edu.pe/discover?scope=%2F&query=',
            'U. La Molina': 'http://repositorio.lamolina.edu.pe/discover?scope=%2F&query=',
        ...
        const paginasComplemento = {
            ...
            'UNMSM': '&submit=Ir',
            'U. Pacífico': '&submit=',
            'U. Cayetano Heredia': '&submit=Ir',
            'U. La Molina': '&submit=Ir'
        ...
        for (let i = 0; i < arrayPages.length; i++) {
            for (let k = 0; k < 1000; k++) {
                if (num[k] != undefined && arrayPages[i] != 'Microsoft Academic' && arrayPages[i] != 'Wws' && arrayPages[i] != 'Cienciasciencegov') {
                    paginasPrincipal[arrayPages[i]] = paginasPrincipal[arrayPages[i]] + num[k] + '+';
                }
                if (arrayPages[i] === 'Microsoft Academic' && num[k] != undefined) {
